...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        setBalance(result.data.balance as unknown as Balance);
        setGold({
  ...
  const fetchPrice = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        setGoldPrice(result.data.buyPrice || result.data.spotPrice || 0);
      }
    } catch (error) {
  ...
  const fetchAll = useCallback(async () => {
    await Promise.all([fetchBalance(), fetchTransactions(), fetchPrice()]);
...
export function useTransactions() {
  const { transactions, isLoadingTransactions, refetchTransactions } =
    useWallet();
