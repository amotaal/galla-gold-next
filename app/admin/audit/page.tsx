...
export default async function AuditLogsPage({
  ...
  if (!['superadmin', 'auditor'].includes(userRole)) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-zinc-400 mt-2">Only Super Admins and Auditors can view audit logs</p>
      </div>
    );
  }
  ...
  const categories = ['all', 'user', 'kyc', 'transaction', 'config', 'system', 'security'];
  const actions = ['all', 'create', 'update', 'delete', 'approve', 'reject', 'suspend', 'login', 'logout'];
            ...
            {totalPages > 1 && (
              <div className="flex justify-between items-center mt-6 pt-6 border-t border-zinc-800">
                <div className="text-sm text-zinc-400">
                  ...
                  {Math.min(filters.page * filters.limit, result.data?.total || 0)} of{' '}
                  {result.data?.total || 0} logs
                </div>
                
                <div className="flex gap-2">
                  {filters.page > 1 && (
                    <a href={`?page=${filters.page - 1}${filters.category ? `&category=${filters.category}` : ''}`}>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-400">
                ...
                Logs cannot be modified or deleted once created. All admin actions 
                are automatically logged with full context including IP address, 
        ...
        <AdminCard>
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-400">
                This audit trail meets regulatory requirements for financial services. 
                All sensitive actions are logged including user management, KYC reviews, 
                transaction modifications, and configuration changes. Regular audits 
        ...
        <AdminCard title="Recent High-Risk Actions" className="mt-6">
          <div className="space-y-3">
            {stats.highRiskActions.map((action: any) => (
              <div key={action._id} className="flex items-center justify-between p-3 bg-red-900/20 border border-red-900/50 rounded-lg">
                <div className="flex items-center space-x-3">
                  <AlertCircle className="w-5 h-5 text-red-400" />
                  <div>
                    <p className="text-sm font-medium text-white">{action.description}</p>
                    <p className="text-xs text-zinc-400">
                      by {action.userEmail} • {format(new Date(action.timestamp), 'MMM dd, HH:mm')}
                    </p>
                  </div>
                </div>
